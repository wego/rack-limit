...
describe Rack::Limit::Backends::Redis do
  ...
  let(:app) { Rack::Limit::Backends::Redis.new target_app, {rules: rules, cache: redis, key_prefix: :throttle} }
  ...
  describe 'rate limit' do
    ...
    it 'should rate limit' do
      ...
      redis.ttl('throttle::127.0.0.1').should_not == -1
    end

    describe 'limit by' do
      let(:rules) { [{'path' => '/api', 'required' => {'params' => {'api_key' => '403 Forbidden (api key required)'}}, 'limit_by' => {'params' => 'api_key'}}] }
